...
console.log('HTH Service native interface vs. HTH JSON RPC interface');
console.log('----------------------------------------------------------------------');
...
var hthd = require('../').services.Hth({
  node: {
    datadir: process.env.HOME + '/.hth',
    network: {
      name: 'testnet'
    }
  }
...
hthd.on('error', function(err) {
  console.error(err.message);
});

hthd.start(function(err) {
  if (err) {
    throw err;
  }
  console.log('HTH Core started');
});

hthd.on('ready', function() {

  console.log('HTH Core ready');

  var client = new HthdRPC({
    host: 'localhost',
    port: 65000,
    user: 'hth',
    pass: 'local321'
  ...
  async.series([
    function(next) {
      ...
      function hthdGetBlockNative(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
        var hash = fixtureData.blockHashes[c];
       hthd.getBlock(hash, function(err, block) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      function hthdGetBlockJsonRpc(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
        var hash = fixtureData.blockHashes[c];
      ...
      function hthGetTransactionNative(deffered) {
        if (c >= txLength) {
          c = 0;
        }
        var hash = fixtureData.txHashes[c];
        hthd.getTransaction(hash, true, function(err, tx) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      function hthGetTransactionJsonRpc(deffered) {
        if (c >= txLength) {
          c = 0;
        }
        var hash = fixtureData.txHashes[c];
      ...
      suite.add('hthd getblock (native)', hthdGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('hthd getblock (json rpc)', hthdGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('hthd gettransaction (native)', hthGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('hthd gettransaction (json rpc)', hthGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
...
>>>>>>> b1cb7c332542ab8c924bc1fe9aa1188d13d723c6
