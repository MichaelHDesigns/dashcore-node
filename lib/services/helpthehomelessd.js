 ...
 */
function Hth(options) {
  if (!(this instanceof Hth)) {
    return new Hth(options);
  }
...
util.inherits(Hth, Service);

Hth.dependencies = [];

Hth.DEFAULT_MAX_TXIDS = 1000;
Hth.DEFAULT_MAX_HISTORY = 50;
Hth.DEFAULT_MAX_UTXO = 1000;
Hth.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Hth.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Hth.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Hth.DEFAULT_SPAWN_RESTART_TIME = 5000;
Hth.DEFAULT_SPAWN_STOP_TIME = 10000;
Hth.DEFAULT_TRY_ALL_INTERVAL = 1000;
Hth.DEFAULT_REINDEX_INTERVAL = 10000;
Hth.DEFAULT_START_RETRY_INTERVAL = 5000;
Hth.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Hth.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Hth.DEFAULT_INSTANTSEND_FEE = 10000;
Hth.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Hth.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Hth.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Hth.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Hth.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Hth.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Hth.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Hth.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Hth.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Hth.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Hth.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Hth.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Hth.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Hth.prototype._initCaches = function() {
...
Hth.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Hth.prototype.getAPIMethods = function() {
  var methods = [
...
Hth.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'hthd/' + name, 'total:', this.subscriptions[name].length);
};

Hth.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'hthd/' + name, 'total:', this.subscriptions[name].length);
};

Hth.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (hthcore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'hthd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Hth.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'hthd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Hth.prototype.unsubscribeAddressAll = function(emitter) {
...
Hth.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Hth.DEFAULT_CONFIG_SETTINGS;
...
Hth.prototype._parseHthConf = function(configPath) {
  var options = {};
...
Hth.prototype._expandRelativeDatadir = function() {
...
Hth.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in hthd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in hthd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in hthd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './hth.conf');

  log.info('Using HTH config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseHthConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseHthConf(networkConfigPath));
  }
...
Hth.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of hthcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of hthcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of hthcore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in hth.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that hthd is undergoing a reindex. ' +
             ...
             'of hthcore-node services will start.');
    node._reindex = true;
  }
};

Hth.prototype._resetCaches = function() {
 ...
 * @private
 */
Hth.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Hth.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Hth.prototype._initChain = function(callback) {
  var self = this;
...
Hth.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9998
  };
  if (this.node.network === hthcore.Networks.testnet) {
    networkOptions.rpcport = 19998;
  }
  return networkOptions;
};

Hth.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === hthcore.Networks.testnet) {
    networkPath = 'testnet3/hth.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/hth.conf';
    }
  }
  return networkPath;
};

Hth.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === hthcore.Networks.testnet) {
...
Hth.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('hthd/hashblock', message.toString('hex'));
    }
  }

};

Hth.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Hth.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('HTH Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Hth.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Hth.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('hthd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Hth.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = hthcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('hthd/rawtransaction', message.toString('hex'));
    }

    var tx = hthcore.Transaction();
    tx.fromString(message);
    var txid = hthcore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Hth.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = hthcore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('hthd/transactionlock', message.toString('hex'));
    }

  }
};

Hth.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Hth.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Hth.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Hth.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Hth.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('HTH Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Hth.DEFAULT_REINDEX_INTERVAL);

};

Hth.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Hth.prototype._stopSpawnedHth = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/hthd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned hth process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean hth process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Hth.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedHth(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting hth process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('HTH process unexpectedly exited with code:', code);
        log.warn('Restarting hth child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('HTH process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn hthd.'));
      }
...
Hth.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to hthd.'));
    }
 ...
 */
Hth.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('HTH configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Hth.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Hth.prototype.syncPercentage = function(callback) {
  var self = this;
...
Hth.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {Function} callback
 */
Hth.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hth.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = hthcore.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
Hth.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Hth.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Hth.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Hth.prototype._getHeightRangeQuery = function(options, clone) {
 ...
 * @param {Function} callback
 */
Hth.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Hth.prototype._getConfirmationsDetail = function(transaction) {
...
Hth.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Hth.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Hth.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Function} callback
 */
Hth.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Hth.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof hthcore.Address) {
...
Hth.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Hth.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Hth.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Hth.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hth.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Hth.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hth.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = hthcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Hth.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hth.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Hth.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Hth.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hth.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Hth.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Hth.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hth.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hth.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param callback
 */
Hth.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Hth.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Hth.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Hth.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Hth.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Hth.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Hth.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Hth.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Hth.prototype.govCount = function (callback) {
  var self = this;
...
Hth.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Hth.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Hth.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Hth.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Hth.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hth.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hth.prototype.getInfo = function(callback) {
  var self = this;
...
Hth.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Hth.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('hthd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('hthd process did not exit'));
      }
...
var async = require('async');
var LRU = require('lru-cache');
var HthdRPC = require('helpthehomelessd-rpc');
var $ = hthcore.util.preconditions;
var _  = hthcore.deps._;
var Transaction = hthcore.Transaction;
var proposal = hthcore.GovObject.Proposal;
...
Hth.prototype._checkConfigIndexes = function(spawnConfig, node) {
  ...
  $.checkState(
    spawnConfig.server && spawnConfig.server === 1,
    '"server" option is required to communicate to helpthehomelessd from hthcore. ' +
      'Please add "server=1" to your configuration and restart'
  );
...
/**
 * Will get the balance for an address or multiple addresses
 * @param {String|Address|Array} addressArg - An address string, hthcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Hth.prototype.getAddressBalance = function(addressArg, options, callback) {
...
Hth.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    ...
    if (clone) {
      // reverse start and end as the order in hthcore is most recent to less recent
      clone.start = options.end;
      clone.end = options.start;
    }
    return true;
...
/**
 * Will retrieve a block as a hthcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Hth.prototype.getBlock = function(blockArg, callback) {
...
module.exports = Hth;
>>>>>>> b9f71dd0ad12d9122965bb2fcda3aae713c1bdae:lib/services/helpthehomelessd.js
