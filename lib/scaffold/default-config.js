...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.hthcore');
  var defaultConfigFile = path.resolve(defaultPath, './hthcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['hthd', 'web'];
...
/**
 * Will return the path and default hthcore-node configuration. It will search for the
 * configuration file in the "~/.hthcore" directory, and if it doesn't exist, it will create one
 * based on default settings.
 * @param {Object} [options]
 * @param {Array} [options.additionalServices] - An optional array of services.
 */
