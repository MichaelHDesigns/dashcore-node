...
function add(options, done) {
  ...
  var hthcoreConfigPath = path.resolve(configPath, 'hthcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(dashcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a hthcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      addService(configPath, service, function(err) {
        ...
        // add service to hthcore-node.json
        addConfig(hthcoreConfigPath, serviceName, next);
      });
    }, done
  );
}
...
function add(options, done) {
  ...
  if (!fs.existsSync(hthcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a hthcore-node.json and/or package.json file.')
    );
  }
