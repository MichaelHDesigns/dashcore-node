...
var BASE_PACKAGE = {
  description: 'A full HTH node build with hthcore',
  repository: 'https://github.com/MicchaelHDesigns/hthcore-node',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'hthcore-lib': '^' + hthcore.version,
    'hthcore-node': version
  }
};
 ...
 */
function createHthDirectory(datadir, done) {
...
/**
 * Will create a base hthcore Node configuration directory and files.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/hthcore-node.json', configJSON);
      if (!isGlobal) {
  ...
  async.series([
    ...
    function(next) {
      ...
      if (!fs.existsSync(absDataDir)) {
        createHthDirectory(absDataDir, next);
      } else {
        next();
      }
    },
